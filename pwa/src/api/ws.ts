import type { Device, WsFrame } from "./types";
...
export class WsClient {
  ...
  private pendingMove = { dx: 0, dy: 0 };
  private pendingScroll = { dx: 0, dy: 0 };
  ...
  sendButton(t: "down" | "up", btn: "left" | "right") {
    this.sendRaw({ t, btn });
  }
  ...
  private scheduleFlush() {
    ...
    this.rafHandle = requestAnimationFrame(() => {
      ...
      if (mx || my) this.sendRaw({ t: "move", dx: mx, dy: my });
      if (sx || sy) this.sendRaw({ t: "scroll", dx: sx, dy: sy });
      this.pendingMove = { dx: 0, dy: 0 };
      this.pendingScroll = { dx: 0, dy: 0 };
    });
  }

  private sendRaw(f: WsFrame) {
